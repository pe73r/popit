const disablePrevNextBtns = (prevBtn, nextBtn, embla) => () => {
  embla.canScrollPrev() ? prevBtn.removeAttribute("disabled") : prevBtn.setAttribute("disabled", "disabled"),
    embla.canScrollNext() ? nextBtn.removeAttribute("disabled") : nextBtn.setAttribute("disabled", "disabled");
};
function throttle(callback, time) {
  this.throttlePause ||
    ((this.throttlePause = !0),
    setTimeout(() => {
      callback(), (this.throttlePause = !1);
    }, time));
}
const lazyLoad = (embla) => {
  ...
  const loadImagesInView = () => {
      embla
        .slidesInView(!0)
        .filter((index) => -1 === imagesInView.indexOf(index))
        .forEach(loadImageInView);
    },
    loadImageInView = (index) => {
      ...
      imagesInView.push(index),
        slideImages.forEach((image) => {
          image.classList.add("opacity-0", "transition-all"),
            ...
            (image.onload = () => {
              image.classList.add("opacity-100"), image.classList.remove("opacity-0");
            });
        });
    };
  return () => (loadImagesInView(), imagesInView.length === images.length);
};
...
function ScrollBounds(limit, location, target, scrollBody) {
  ...
  return { constrain: constrain, toggleActive: toggleActive };
}
...
function Engine(root, container, slides, options, events) {
  var align = options.align,
    ...
    contain,
    scrollSnaps = loop || "" === containScroll ? snapsAligned : snapsContained,
...
customElements.define("carousel-container", class CC extends H {}),
  customElements.define("carousel-next", class CN extends H {}),
  customElements.define("carousel-prev", class CP extends H {}),
  customElements.define("carousel-dots", class CP extends H {}),
  customElements.define("carousel-dot", class CP extends H {});
const setupDotBtns = (dotsArray, embla) =>
    dotsArray.forEach((dotNode, i) => dotNode.addEventListener("click", () => embla.scrollTo(i), !1)),
  generateDotBtns = (carousel, embla) => {
    ...
    return (
      embla.scrollSnapList().forEach((d, i) => {
        const newDot = dot.cloneNode(!0);
        newDot.setAttribute("aria-selected", i === selected ? "true" : "false"), dots.appendChild(newDot);
      }),
...
customElements.define(
  ...
  class Carousel extends HTMLElement {
    connectedCallback() {
      const carousel = this,
        observerContainer = new MutationObserver((mutations) => {
          const container = carousel.querySelector("carousel-container");
          container && (carousel.init(), observerContainer.disconnect());
        });
      observerContainer.observe(this, { childList: !0 });
    }
    parseResponsiveProp = (attribute) => {
      ...
      if (isNaN(toNumber)) {
        const responsiveProps = eval(`({${value.split(" ").join(",")}})`),
          sizes = { "2xl": 1536, xl: 1280, lg: 1024, md: 768, sm: 640 },
          applicablesSizes = Object.entries(sizes).reduce(
            (acc, [size, breakpoint]) => (breakpoint <= window.innerWidth && acc.push(size), acc),
            []
          ),
          parsedValue = responsiveProps[applicablesSizes.find((size) => responsiveProps[size])];
        return parsedValue || Object.values(responsiveProps)[0];
      }
      return toNumber;
    };
    buildPanels = () => {
      const slidesToShow = this.parseResponsiveProp("slidesToShow"),
        panels = this.querySelectorAll("carousel-content > div");
      panels.forEach((panel) => {
        const width = (100 / slidesToShow).toFixed(1);
        (panel.style.flex = `0 0 ${width}%`), (panel.style.minWidth = `${width}%`), (panel.style.position = "relative");
      });
    };
    handleDots = () => {
      ...
      dotsArray &&
        (this.setSelectedDotBtn &&
          (this.embla.off("select", this.setSelectedDotBtn), this.embla.off("init", this.setSelectedDotBtn)),
    ...
    init = () => {
      const loop = "true" === this.getAttribute("loop"),
        container = this.querySelector("carousel-container");
      this.buildPanels();
      const options_align = "start",
        options_loop = loop,
        options_skipSnaps = !0;
      setTimeout(() => {
        ...
        const next = document.querySelector("carousel-next"),
          prev = this.querySelector("carousel-prev");
        this.embla.reInit(),
          next && next.addEventListener("click", this.embla.scrollNext),
          prev && prev.addEventListener("click", this.embla.scrollPrev);
        ...
        if (loadImagesInView) {
          const loadImagesInViewAndDestroyIfDone = (eventName) => {
            ...
            loadedAll && this.embla.off(eventName, loadImagesInViewAndDestroyIfDone);
          };
        ...
        if ((this.handleDots(), !loop)) {
          const disablePrevAndNextBtns = disablePrevNextBtns(prev, next, this.embla);
          this.embla.on("init", disablePrevAndNextBtns), this.embla.on("select", disablePrevAndNextBtns);
        }
        window.addEventListener(
          "resize",
          () =>
            throttle.bind(this)(() => {
              console.log("throttle"), this.buildPanels(), this.embla.reInit(), this.handleDots();
            }, 200),
          { passive: !0 }
        );
      }, 300);
