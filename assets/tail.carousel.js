const autoPlay = (carousel, delay) => {
  ...
  return window.setInterval(scrollNext, delay);
};

const lazyLoad = (embla) => {
  const images = embla.slideNodes().map((slide) => slide.querySelectorAll("img[data-src]"));
  const imagesInView = [];
  if (!images.length) return;

  const loadImagesInView = () => {
    const slidesInView = embla.slidesInView(true);
    const slidesInViewPlusOne = slidesInView.concat([slidesInView[slidesInView.length - 1] + 1]);

    slidesInViewPlusOne.filter((index) => imagesInView.indexOf(index) === -1).forEach(loadImageInView);
  };

  const loadImageInView = (index) => {
    const slideImages = images[index];

    imagesInView.push(index);
    if (slideImages) {
      slideImages.forEach((image) => {
        image.classList.add("opacity-0", "transition-all");
...
if (!customElements.get("tail-carousel")) {
  ...
  const generateDotBtns = (carousel, embla, manualDots) => {
    if (!manualDots) {
  ...
  customElements.define(
    ...
    class Carousel extends HTMLElement {
      ...
      connectedCallback() {
        const carousel = this;
        const maybePanel = carousel.querySelector("carousel-panel");
        if (maybePanel) {
          return carousel.init();
        }
        const observerContainer = new MutationObserver(() => {
          if (carousel.querySelector("carousel-panel")) {
            carousel.init();
      ...
      handleDots = () => {
        const dotsArray = generateDotBtns(this, this.embla, this.getAttribute("data-auto-dots") === "false");
      ...
      disablePrevNext = () => {
        if (this.embla.canScrollPrev() && this.prev) this.prev.removeAttribute("disabled");
        else if (this.prev) this.prev.setAttribute("disabled", "true");

        if (this.embla.canScrollNext() && this.next) this.next.removeAttribute("disabled");
        else if (this.next) this.next.setAttribute("disabled", "true");
      };
      handleArrows = () => {
      ...
      init = () => {
        ...
        setTimeout(() => {
          ...
          const autoplayDelay = this.getAttribute("data-auto-play");
          if (autoplayDelay) {
            if (this.timer) {
              window.clearInterval(this.timer);
            }
            this.timer = autoPlay(this.embla, Number(autoplayDelay));
          }
          ...
          // LAZYLOADING
          const loadImagesInView = lazyLoad(this.embla);
          if (loadImagesInView) {
            const loadImagesInViewAndDestroyIfDone = (eventName) => {
              const loadedAll = loadImagesInView();
              if (loadedAll) this.embla.off(eventName, loadImagesInViewAndDestroyIfDone);
            };
